...
import com.vibhuti.gaming.sprites.Enemy;
import com.vibhuti.gaming.sprites.Player;
...
public class Board extends JPanel {
	...
	private void gameOver(Graphics pen) {
			if(player.outOfScreen()) {
                ...
                pen.drawString("Game Win", 1500, 950/2);
                timer.stop();
                return;
	        ...
	        for (Enemy enemy : enemies) {
	            if (isCollide(enemy)) {
	            	pen.setFont(new Font("times",Font.BOLD,30));
	            	pen.setColor(Color.RED);
	                pen.drawString("Game Over", 1500, 950/2);
	                timer.stop();
	        }
	    }
	}
	...
	private void loadBackgroundImage() {
		try {
			backgroundImage = ImageIO.read(Board.class.getResource("board game.jpg"));
		} catch (IOException e) {
